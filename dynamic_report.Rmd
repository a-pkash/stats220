...
Overall (not based on direction), the 

If we compare the crowded-ness to the average duration, we can get more insight 
...
From the plot above, we can see that the bus commute that takes the longest tends to 
be the journey from `r longest_overall$direction` when taking the `r longest_overall$bus_route`. 
This takes `r longest_overall$avg_duration` minutes on average, and the average start time 
of the recorded journeys using this route is `r longest_overall$avg_start_time`.

We can also discern that the bus commute that takes the least time tends to 
be the journey from `r shortest_overall$direction` when taking the `r shortest_overall$bus_route`. 
This takes `r longest_overall$avg_duration` minutes on average, and the average start time 
of the recorded journeys using this route is `r shortest_overall$avg_start_time`.
